...
client_id, secret = open('python_data/keys/client_andreas.secret').read().strip().split(',')
port = 5000
...
class MyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
  ...
  def do_GET(self):
    ...
    df_total.to_csv('python_data/strava_andreas_' + str(now.strftime('%Y%m%d%H%M%S')) + '.csv')
